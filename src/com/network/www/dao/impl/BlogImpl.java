...
public class BlogImpl implements BlogInf {
    private String table_name = SystemUtil.BlogTable;
	@Override
	...
	public boolean publishBlog(NetworkBlog blog) {
		...
		String sql = "insert into "+ table_name +" (blog_title,blog_content,blog_user,blog_date,blog_up_count,blog_down_count) values(?,?,?,?,?,?)";
		try {
	...
	public boolean upMyBlog(int blog_id, int upcount) {
		...
		Connection conn = JDBCUtil.getConn();
		PreparedStatement ppst = null;
		String sql = "update "+ table_name +" set blog_up_count=? where id=?";
		try {
			ppst = conn.prepareStatement(sql);
	...
	public boolean downMyBlog(int blog_id, int downcount) {
		...
		Connection conn = JDBCUtil.getConn();
		PreparedStatement ppst = null;
		String sql = "update "+ table_name +" set blog_down_count=? where id=?";
		try {
			ppst = conn.prepareStatement(sql);
